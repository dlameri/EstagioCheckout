...
public class ApiShoppingCartController {
    ...
    @RequestMapping("/cart")
    public @JsonValue @JsonRawValue@ResponseBody String listCart(@CookieValue(value="ShoppingCartItems", required=false) String cartCookie) {
    	try {
			ShoppingCart shoppingCart = apiShoppingCartService.getShoppingCart(cartCookie);
			return apiShoppingCartService.getCartJson(shoppingCart);
